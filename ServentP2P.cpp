...
class ServentP2P {
	...
	char server_ip[30];
	static char shared_dir[100];
	pthread_t alive_thread;
	pthread_t send_files_thread;

	/* identifica se o cliente já conectou no servidor */
	static bool has_joined;

	public:

	ServentP2P(const char *ip, const char *dir) {
		if(strlen(ip) > 30) {
			exit(-1);
		}
		strcpy(server_ip, ip);
		strcpy(shared_dir, dir);
		...
		pthread_create(&alive_thread, NULL, &ServentP2P::alive, server_ip);

		/* cria thread para receber requisicoes de arquivos */
		pthread_create(&send_files_thread, NULL, &ServentP2P::send_files, shared_dir);
	};
	...
	/*
	 * Pega um arquivo em outro servent.
	 *
	 */
	...
	static bool send_file(string &fname, ServerSocket &sock) {
		string fpath = string(ServentP2P::shared_dir) + 
								string("/") + fname;
...
bool ServentP2P::has_joined = false;
char ServentP2P::shared_dir[100];
